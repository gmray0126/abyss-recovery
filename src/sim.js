...
function blocked(x,y,r){if(x-r<0||y-r<0||x+r>WORLD_W||y+r>WORLD_H)return true;return MAP_OBSTACLES.some(o=>circleRect(x,y,r,o))}
...
function visionParams(level=3){const near=105+level*32;return{near,far:near+400+level*48,half:1.0}}
...
function chooseCover(a,target){let best=null,score=Infinity;for(const c of COVER_POINTS){const da=distXY(a.x,a.y,c.x,c.y);if(da>470||blocked(c.x,c.y,a.r))continue;if(!segmentBlocked(c.x,c.y,target.x,target.y))continue;const dt=distXY(target.x,target.y,c.x,c.y),s=da+Math.abs(300-dt)*.35;if(s<score){score=s;best=c}}return best}
function steerToward(a,tx,ty,speed,dt){const n=norm(tx-a.x,ty-a.y);if(moveEntity(a,n.x*speed*dt,n.y*speed*dt))return;const s=a.strafe||1;if(!moveEntity(a,-n.y*s*speed*.72*dt,n.x*s*speed*.72*dt))a.strafe*=-1}
...
function aiStep(w,a,dt){if(a.dead)return;a.fireCd=Math.max(0,a.fireCd-dt);a.flinch=Math.max(0,a.flinch-dt);a.grenadeCd=Math.max(0,a.grenadeCd-dt);if(a.reload>0){a.reload-=dt;if(a.reload<=0)finishReload(a)}const humans=w.humanIds.map(id=>getPlayer(w,id)).filter(Boolean),nearHuman=humans.some(p=>!p.dead&&dist(a,p)<1500);if(!nearHuman&&!a.targetId&&!a.heard){a.sleepAcc+=dt;if(a.sleepAcc<.28)return;dt=a.sleepAcc;a.sleepAcc=0}else a.sleepAcc=0;
 a.thinkCd-=dt;if(a.thinkCd<=0){a.thinkCd=a.kind==='pmc'?.22:.34;const vis=nearestVisibleHostile(w,a,a.kind==='pmc'?820:610);if(vis){a.targetId=vis.id;a.lastKnown={x:vis.x,y:vis.y,time:w.time};if(a.kind==='pmc')a.cover=chooseCover(a,vis)}else{const h=recentHeard(w,a);if(h){a.heard={x:h.x,y:h.y,time:w.time};a.targetId=null}else if(a.lastKnown&&w.time-a.lastKnown.time>5)a.lastKnown=null}}
 const t=getActor(w,a.targetId);if(t&&!t.dead&&canSee(a,t,a.kind==='pmc'?4:2)){const gun=currentWeapon(a),def=WEAPONS[gun.type],d=dist(a,t),desired=gun.type==='shotgun'?145:gun.type==='dmr'?520:a.kind==='pmc'?330:250;a.angle=Math.atan2(t.y-a.y,t.x-a.x);
   ...
   if(a.cover&&distXY(a.x,a.y,a.cover.x,a.cover.y)>42)steerToward(a,a.cover.x,a.cover.y,a.moveSpeed,dt);else if(d>desired+80)steerToward(a,t.x,t.y,a.moveSpeed,dt);else if(d<desired-70){const n=norm(t.x-a.x,t.y-a.y);moveEntity(a,-n.x*a.moveSpeed*.7*dt,-n.y*a.moveSpeed*.7*dt)}else{const n=norm(t.x-a.x,t.y-a.y);moveEntity(a,-n.y*a.strafe*a.moveSpeed*.28*dt,n.x*a.strafe*a.moveSpeed*.28*dt)}
   if(d<Math.min(def.range*.82,a.kind==='pmc'?760:560)&&!segmentBlocked(a.x,a.y,t.x,t.y))aiFire(w,a,t);if(gun.ammo<=0)startReload(a);return}
 const goal=a.lastKnown||a.heard||a.patrol;if(goal){a.angle=Math.atan2(goal.y-a.y,goal.x-a.x);if(distXY(a.x,a.y,goal.x,goal.y)>45)steerToward(a,goal.x,goal.y,a.moveSpeed*.72,dt);else{if(a.heard)a.heard=null;if(a.lastKnown)a.lastKnown=null;a.patrol={...PATROLS[Math.floor(Math.random()*PATROLS.length)]}}}}

function updateBullets(w,dt){for(const b of w.bullets){let alive=true;const steps=Math.max(1,Math.ceil(Math.hypot(b.vx,b.vy)*dt/24)),sd=dt/steps;for(let s=0;s<steps&&alive;s++){b.x+=b.vx*sd;b.y+=b.vy*sd;b.life-=sd;if(b.life<=0||b.x<0||b.y<0||b.x>WORLD_W||b.y>WORLD_H){alive=false;break}if(MAP_OBSTACLES.some(o=>b.x>o.x&&b.x<o.x+o.w&&b.y>o.y&&b.y<o.y+o.h)){emit(w,'impact',{x:b.x,y:b.y});alive=false;break}for(const a of w.actors){if(a.dead||a.id===b.owner)continue;if(distXY(b.x,b.y,a.x,a.y)<a.r+3){const hs=Math.random()<b.headChance;damageActor(w,a,b.damage,b.owner,hs);alive=false;break}}}b.alive=alive}w.bullets=w.bullets.filter(b=>b.alive)}
function updateGrenades(w,dt){for(const g of w.grenades){g.fuse-=dt;g.vx*=Math.pow(.64,dt);g.vy*=Math.pow(.64,dt);const nx=g.x+g.vx*dt,ny=g.y+g.vy*dt;if(blocked(nx,g.y,7))g.vx*=-.48;else g.x=nx;if(blocked(g.x,ny,7))g.vy*=-.48;else g.y=ny;if(g.fuse<=0){for(const a of w.actors){if(a.dead)continue;const d=distXY(g.x,g.y,a.x,a.y);if(d>g.radius)continue;const cover=segmentBlocked(g.x,g.y,a.x,a.y),fall=1-d/g.radius,raw=g.damage*(.35+.65*fall)*(cover?.22:1);damageActor(w,a,raw,g.owner,false)}emit(w,'explosion',{x:g.x,y:g.y,radius:g.radius,owner:g.owner});addSound(w,g.x,g.y,1350,'explosion',g.owner);g.dead=true}}w.grenades=w.grenades.filter(g=>!g.dead)}
...
export function buildSnapshot(w,viewerId){const p=getPlayer(w,viewerId);if(!p)return null;const visible=w.actors.filter(a=>a.id===viewerId||(!a.dead&&canSee(p,a,p.visionLevel))).map(actorPublic);const openId=w.openContainers[viewerId],oc=getContainer(w,openId);const interaction=w.interactions[viewerId]?copy(w.interactions[viewerId]):null;const gear={};for(const k of ['primary','secondary','armor','helmet','backpack'])gear[k]=publicItem(p.gear[k]);const gun=currentWeapon(p);return{time:w.time,timeLeft:w.timeLeft,worldW:WORLD_W,worldH:WORLD_H,obstacles:w.obstacles,zones:w.zones,extracts:w.extracts,crates:w.crates.map(c=>({id:c.id,x:c.x,y:c.y,opened:c.opened,empty:c.items.length===0})),corpses:w.corpses.map(c=>({id:c.id,x:c.x,y:c.y,ownerName:c.ownerName,empty:c.items.length===0})),actors:visible,bullets:w.bullets.filter(b=>distXY(p.x,p.y,b.x,b.y)<1100).map(b=>({x:b.x,y:b.y,vx:b.vx,vy:b.vy,owner:b.owner})),grenades:w.grenades.filter(g=>distXY(p.x,p.y,g.x,g.y)<1100).map(g=>({x:g.x,y:g.y,fuse:g.fuse})),playerId:viewerId,aliveAI:w.actors.filter(a=>a.kind!=='player'&&!a.dead).length,player:{id:p.id,x:p.x,y:p.y,hp:p.hp,maxHp:p.maxHp,angle:p.angle,kills:p.kills,visionLevel:p.visionLevel,weaponSlot:p.weaponSlot,weapon:publicItem(gun),gear,inventory:p.inventory.map(publicItem),bagCapacity:bagCapacity(p),armor:p.gear.armor?.armor||0,maxArmor:p.gear.armor?.maxArmor||p.gear.armor?.armor||0,reload:p.reload},openContainer:oc?{id:oc.id,kind:oc.kind,ownerName:oc.ownerName,items:oc.items.map(publicItem)}:null,interaction,result:w.results[viewerId]?copy(w.results[viewerId]):null};}

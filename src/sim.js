...
export const AMMO_STACK_MAX=999;
...
export const WEAPON_PARTS={
 ...
 tactical_stock:{kind:'part',key:'tactical_stock',slot:'stock',name:'전술 개머리판',price:460,compatibleClasses:['smg','ar','battle','shotgun','dmr'],recoilMult:.80,spreadMult:.90},
 heavy_stock:{kind:'part',key:'heavy_stock',slot:'stock',name:'중량 개머리판',price:570,compatibleClasses:['ar','battle','shotgun','dmr','sniper'],recoilMult:.65,moveMult:.90},
 ...
 choke:{kind:'part',key:'choke',slot:'muzzle',name:'정밀 초크',price:410,compatibleClasses:['shotgun'],spreadMult:.66,recoilMult:1.04},
 breacher:{kind:'part',key:'breacher',slot:'muzzle',name:'브리처 총구',price:460,compatibleClasses:['shotgun'],spreadMult:1.12,recoilMult:.78,soundMult:1.10},
...
function portMap(){
 ...
 return{
  id:'harbor',name:'침수 항만 09',trait:'침수 항만 · 넓은 사선 / 물속 이동 둔화',visionMod:1,waterPassable:true,waterSpeed:.62,dynamicCoverBase:18,lootProfile:'harbor',palette:MAP_PALETTES.harbor,
...
function researchMap(){
 ...
 return{
  id:'research',name:'오로라 폐쇄 연구기지',trait:'정전 연구기지 · 시야 감소 / 고급 파츠 출현',visionMod:-1,waterPassable:false,waterSpeed:1,dynamicCoverBase:14,lootProfile:'research',palette:MAP_PALETTES.research,
...
function coverPlacementBlocked(c,fixed,placed,def){
 ...
 for(const p of def.patrols)if(rectOverlap(c,safeRect(p.x,p.y,96)))return true;
 return false
}
function buildRaidObstacles(def,seed){
 ...
 const extraCount=(def.dynamicCoverBase??28)+Math.floor(rng()*7),shapes=[...movable];
...
function partItem(key){const d=WEAPON_PARTS[key];return d?{id:uid('pt'),...copy(d),value:d.price}:null}
...
function ammoStackMatch(a,b){return a?.kind==='ammo'&&b?.kind==='ammo'&&a.caliber===b.caliber&&(a.tier||1)===(b.tier||1)&&!!a.starterIssue===!!b.starterIssue}
function syncAmmoStackValue(it){if(it?.kind!=='ammo')return it;it.qty=Math.max(0,Math.floor(Number(it.qty)||0));it.value=it.starterIssue?0:Math.round(it.qty*(AMMO_TIERS[it.tier||1]?.unitPrice||3));return it}
function addItemToInventoryStacked(inventory,raw,maxSlots=Infinity){const item=normalizeItem(raw);if(item.kind!=='ammo'){if(inventory.length>=maxSlots)return{movedQty:0,remainder:1};inventory.push(item);return{movedQty:1,remainder:0}}let remaining=Math.max(0,item.qty||0),start=remaining;for(const stack of inventory){if(remaining<=0)break;if(!ammoStackMatch(stack,item)||(stack.qty||0)>=AMMO_STACK_MAX)continue;const add=Math.min(remaining,AMMO_STACK_MAX-(stack.qty||0));stack.qty=(stack.qty||0)+add;remaining-=add;syncAmmoStackValue(stack)}while(remaining>0&&inventory.length<maxSlots){const add=Math.min(remaining,AMMO_STACK_MAX);inventory.push(syncAmmoStackValue({...item,id:uid('a'),qty:add}));remaining-=add}return{movedQty:start-remaining,remainder:remaining}}
function compactInventoryStacks(items,maxSlots=Infinity){const out=[];for(const it of items||[])addItemToInventoryStacked(out,it,maxSlots);return out}
...
function solidObstacle(w,o){return !(o.kind==='water'&&w?.map?.waterPassable)}
...
function makeActor(kind,x,y,team,load={}){
 ...
 const inventory=compactInventoryStacks(load.inventory||[]);
 return{id:uid(kind[0]),kind,team,name:load.name||kind.toUpperCase(),x,y,r:13,hp:100,maxHp:100,gear:{primary,secondary,armor,helmet,backpack},weaponSlot:'primary',inventory,angle:0,moveSpeed:kind==='scav'?126:150,fireCd:0,reload:0,input:emptyInput(),triggerHeld:false,dead:false,kills:0,thinkCd:Math.random()*.3,patrol:{...(load.patrols||PATROLS)[Math.floor(Math.random()*(load.patrols||PATROLS).length)]},targetId:null,lastKnown:null,heard:null,cover:null,strafe:Math.random()<.5?-1:1,stepCd:Math.random()*.4,healCd:0,healTimer:0,healDuration:0,healItemId:null,flinch:0,grenadeCd:5+Math.random()*6,sleepAcc:0,interactLatch:false,visionLevel:load.visionLevel||3,recoil:0,secureItemId:load.secureItemId||null};
}

function rollCrate(i,def={}){const out=[];const names=def.lootProfile==='harbor'?['밀수 전자부품','항만 송수신기','선박 제어모듈','봉인 화물','냉동 의약품']:def.lootProfile==='research'?['실험 데이터 SSD','정밀 광학 모듈','연구 샘플','서버 제어보드','보안 키카드']:['암호화 SSD','군용 무전기','광학 부품','공구 세트','의약품 박스','배터리 팩','제어 모듈'],cals=['9mm','5.56','7.62','12g'];const valueMult=def.lootProfile==='harbor'?1.15:def.lootProfile==='research'?1.08:1;out.push(valuable(names[i%names.length],Math.round((260+Math.floor(Math.random()*650))*valueMult)));if(Math.random()<.72){const caliber=cals[i%cals.length],bonus=def.lootProfile==='research'&&Math.random()<.55?1:0,tier=Math.min(5,1+bonus+Math.floor(Math.pow(Math.random(),1.7)*5)),pack=AMMO_CALIBERS[caliber].pack;out.push(ammoItem(caliber,Math.max(6,Math.round(pack*(.55+Math.random()*.7))),tier))}if(Math.random()<.38)out.push(gearItem('med'));if(Math.random()<.24)out.push(gearItem('grenade'));if(Math.random()<(def.lootProfile==='research'?.36:def.lootProfile==='harbor'?.22:.16)){const keys=Object.keys(WEAPON_PARTS),pt=partItem(keys[(i+Math.floor(Math.random()*keys.length))%keys.length]);if(pt)out.push(pt)}if(Math.random()<.15){const guns=['pistol','smg','mp5','vector','ar','m4','scar','shotgun','saiga','dmr','m700'];out.push(weaponItem(guns[(i+Math.floor(Math.random()*guns.length))%guns.length],1+.04*Math.random(),1+Math.floor(Math.random()*2)))}return out}
...
function spawnFillerAI(w,index=0){
 ...
 const pmc=(w.aiSpawnSeq%4===0),kind=pmc?'pmc':'scav',guns=pmc?['ar','m4','scar','dmr','m700','vector']:['smg','mp5','shotgun','saiga','pistol','ar'];
...
export function addHumanPlayer(w,pc={}){
 const s=pickHumanSpawn(w),team=`human-${uid('team')}`,a=makeActor('player',s.x,s.y,team,{name:pc.name||'PLAYER',primary:pc.equipment?.primary,secondary:pc.equipment?.secondary,armor:pc.equipment?.armor,helmet:pc.equipment?.helmet,backpack:pc.equipment?.backpack,inventory:pc.inventory||[],visionLevel:pc.visionLevel||3,secureItemId:pc.secureItemId||null});
...
export function createWorld(config={}){
 const players=config.players||[{}],def=selectedMap(config.mapIndex||0),layout=buildRaidObstacles(def,config.coverSeed),map={id:def.id,name:def.name,trait:def.trait||'',visionMod:def.visionMod||0,waterPassable:!!def.waterPassable,waterSpeed:def.waterSpeed||1,lootProfile:def.lootProfile||'industrial',palette:copy(def.palette),roads:copy(def.roads),layoutSeed:layout.seed,dynamicCoverCount:layout.dynamicCount};
 ...
 def.crates.forEach((q,i)=>w.crates.push({id:`crate-${i}`,kind:'crate',x:q.x,y:q.y,r:19,opened:false,items:rollCrate(i,def)}));return w
}
...
function effectiveVisionLevel(w,a){const gun=currentWeapon(a),parts=weaponPartEffects(gun);return clamp((a.visionLevel||3)+(w.map?.visionMod||0)+(parts.visionBonus||0),1,10)}
...
function updateHealing(w,p,dt){if(p.healTimer<=0)return;p.healTimer=Math.max(0,p.healTimer-dt);if(p.healTimer>0)return;const i=p.inventory.findIndex(x=>x.id===p.healItemId&&x.kind==='med');p.healItemId=null;if(i<0)return;const m=p.inventory.splice(i,1)[0],before=p.hp;if(p.secureItemId===m.id)p.secureItemId=null;p.hp=Math.min(p.maxHp,p.hp+(m.heal||45));emit(w,'heal',{actor:p.id,amount:Math.round(p.hp-before)})}
export function applyAction(w,id,action){const p=getPlayer(w,id);if(!p||p.dead||w.results[id])return;if(action.type===ACTIONS.CLOSE_CONTAINER)delete w.openContainers[id];else if(action.type===ACTIONS.LOOT){const c=getContainer(w,action.containerId||w.openContainers[id]);if(!c||dist(p,c)>145)return;const i=c.items.findIndex(x=>x.id===action.itemId);if(i<0)return;const source=c.items[i];if(source.kind==='ammo'){const moved=addItemToInventoryStacked(p.inventory,source,bagCapacity(p));if(!moved.movedQty)return;source.qty=moved.remainder;syncAmmoStackValue(source);if(source.qty<=0)c.items.splice(i,1);w.openContainers[id]=c.id;emit(w,'loot',{recipient:id,item:`${source.name} +${moved.movedQty}발`,itemId:source.id})}else{if(p.inventory.length>=bagCapacity(p))return;const it=c.items.splice(i,1)[0];p.inventory.push(normalizeItem(it));w.openContainers[id]=c.id;emit(w,'loot',{recipient:id,item:it.name,itemId:it.id})}}else if(action.type===ACTIONS.EQUIP_BAG){if(applyBagEquip(p,action.slot))emit(w,'equip',{recipient:id,item:currentWeapon(p)?.name||'장비'})}else if(action.type===ACTIONS.SWAP_WEAPON){const slot=action.slot==='secondary'?'secondary':'primary';if(p.gear[slot]){p.weaponSlot=slot;p.reload=0;emit(w,'equip',{recipient:id,item:p.gear[slot].name})}}else if(action.type===ACTIONS.SECURE_ITEM){const it=p.inventory.find(x=>x.id===action.itemId);if(!it)return;p.secureItemId=p.secureItemId===it.id?null:it.id;emit(w,'secure',{recipient:id,item:it.name||'아이템',active:!!p.secureItemId})}else if(action.type===ACTIONS.DISCARD_ITEM){const i=p.inventory.findIndex(x=>x.id===action.itemId);if(i>=0){const [it]=p.inventory.splice(i,1);if(p.secureItemId===it.id)p.secureItemId=null;emit(w,'discard',{recipient:id,item:it.name||'아이템'})}}else if(action.type===ACTIONS.GRENADE)throwGrenade(w,p);else if(action.type===ACTIONS.USE_MED)useMed(w,p)}

function movePlayer(w,p,dt){const inp=w.inputs[p.id]||emptyInput();updateHealing(w,p,dt);p.recoil=Math.max(0,(p.recoil||0)-dt*.050);const healing=p.healTimer>0,v=norm(inp.moveX,inp.moveY);if(inp.moveX||inp.moveY){const sp=p.moveSpeed*weaponPartEffects(currentWeapon(p)).moveMult*(inp.sprint&&!healing?1.48:1)*(healing?0.55:1);moveEntity(w,p,v.x*sp*dt,v.y*sp*dt);p.stepCd-=dt;if(p.stepCd<=0){p.stepCd=(inp.sprint&&!healing)?0.36:0.58;const wet=pointInWater(w,p.x,p.y);addSound(w,p.x,p.y,(inp.sprint&&!healing?380:135)*(wet?1.55:1),'footstep',p.id)}}p.angle=Math.atan2(inp.aimY,inp.aimX);p.fireCd=Math.max(0,p.fireCd-dt);p.flinch=Math.max(0,p.flinch-dt);if(p.reload>0){p.reload-=dt;if(p.reload<=0)finishReload(p)}if(inp.reload&&!healing)startReload(p);const gun=currentWeapon(p),def=gun?WEAPONS[gun.type]:null;if(inp.shoot&&def&&!healing&&(def.auto||!p.triggerHeld))shoot(w,p,inp.aimX,inp.aimY,false);p.triggerHeld=!!inp.shoot&&!healing;updateInteraction(w,p,dt)}
...
function aiStep(w,a,dt){if(a.dead)return;ensureActorFree(w,a);a.recoil=Math.max(0,(a.recoil||0)-dt*.060);a.fireCd=Math.max(0,a.fireCd-dt);a.flinch=Math.max(0,a.flinch-dt);a.grenadeCd=Math.max(0,a.grenadeCd-dt);a.doorCd=Math.max(0,(a.doorCd||0)-dt);if(a.reload>0){a.reload-=dt;if(a.reload<=0)finishReload(a)}const humans=w.humanIds.map(id=>getPlayer(w,id)).filter(Boolean),nearHuman=humans.some(p=>!p.dead&&dist(a,p)<1500);if(!nearHuman&&!a.targetId&&!a.heard){a.sleepAcc+=dt;if(a.sleepAcc<.28)return;dt=a.sleepAcc;a.sleepAcc=0}else a.sleepAcc=0;
 ...
 const t=getActor(w,a.targetId);if(t&&!t.dead&&canSee(w,a,t,a.kind==='pmc'?4:2)){const gun=currentWeapon(a),def=WEAPONS[gun.type],d=dist(a,t),desired=def.class==='shotgun'?150:def.class==='sniper'?650:def.class==='dmr'?520:def.class==='battle'?410:a.kind==='pmc'?330:250;a.angle=Math.atan2(t.y-a.y,t.x-a.x);
 ...
 const goal=a.lastKnown||a.heard||a.patrol;if(goal){a.angle=Math.atan2(goal.y-a.y,goal.x-a.x);if(distXY(a.x,a.y,goal.x,goal.y)>45)steerToward(w,a,goal.x,goal.y,a.moveSpeed*.72,dt);else{if(a.heard)a.heard=null;if(a.lastKnown)a.lastKnown=null;a.patrol={...(w.patrols||PATROLS)[Math.floor(Math.random()*(w.patrols||PATROLS).length)]}}}}

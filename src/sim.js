...
function makeActor(kind,x,y,team,load={}){
 ...
 return{id:uid(kind[0]),kind,team,name:load.name||kind.toUpperCase(),x,y,r:13,hp:100,maxHp:100,gear:{primary,secondary,armor,helmet,backpack},weaponSlot:'primary',inventory,angle:0,moveSpeed:kind==='scav'?126:150,fireCd:0,reload:0,input:emptyInput(),triggerHeld:false,dead:false,kills:0,thinkCd:Math.random()*.3,patrol:{...(load.patrols||PATROLS)[Math.floor(Math.random()*(load.patrols||PATROLS).length)]},targetId:null,lastKnown:null,heard:null,cover:null,strafe:Math.random()<.5?-1:1,stepCd:Math.random()*.4,healCd:0,healTimer:0,healDuration:0,healItemId:null,flinch:0,grenadeCd:5+Math.random()*6,sleepAcc:0,interactLatch:false,visionLevel:load.visionLevel||3,recoil:0};
}

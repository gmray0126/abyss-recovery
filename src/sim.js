...
function useMed(w,p){if(p.hp>=p.maxHp)return;const i=p.inventory.findIndex(x=>x.kind==='med');if(i<0)return;const m=p.inventory.splice(i,1)[0],before=p.hp;p.hp=Math.min(p.maxHp,p.hp+(m.heal||45));emit(w,'heal',{actor:p.id,amount:Math.round(p.hp-before)})}

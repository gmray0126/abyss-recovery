...
function loadResume(){
  try{
    ...
    if(data.endsAt&&Date.now()>data.endsAt+DEFAULT_RECONNECT_GRACE_MS)return{...data,expired:true};
    return data;
  }catch{return null}
}
...
export class WebSocketSession extends Emitter{
 constructor(url){super();const saved=loadResume();this.url=url;this.ws=null;this.manual=false;this.joined=false;this.pendingJoin=null;this.lastSnapshot=null;this.reconnectTimer=null;this.disconnectTimer=null;this.expiredRecovery=saved?.expired?saved.recovery||null:null;this.resumeToken=saved?.expired?null:(saved?.token||null);this.resumeInfo=saved?.expired?null:(saved||null);this.reconnectGraceMs=Number(saved?.reconnectGraceMs)||DEFAULT_RECONNECT_GRACE_MS;if(saved?.expired)clearResume();this.connect()}
 get connected(){return this.ws?.readyState===1}
 takeExpiredSecureItem(){const r=this.expiredRecovery;this.expiredRecovery=null;if(!r?.secureItemId)return null;return r.inventory?.find(i=>i.id===r.secureItemId)||null}
 _recoverySecureItem(){const r=this.resumeInfo?.recovery;if(!r?.secureItemId)return null;return r.inventory?.find(i=>i.id===r.secureItemId)||null}

...
function handleEvent(e){
 ...
 if(e.type==='loot')toast(`획득: ${e.item}`);if(e.type==='equip')toast(`장착: ${e.item}`);
}
...
function renderBag(){const g=$('#bagGrid');g.innerHTML='';const inv=snap.player.inventory;for(let i=0;i<snap.player.bagCapacity;i++){const it=inv[i],d=document.createElement('div');d.className='bagSlot';if(it){d.innerHTML=`<b>${itemName(it)}</b><small>${itemSub(it)}</small>`;if(['weapon','armor','helmet','backpack'].includes(it.kind)){const b=document.createElement('button');b.textContent='장착';b.onclick=()=>session?.sendAction({type:ACTIONS.EQUIP_BAG,slot:i});d.appendChild(b)}}g.appendChild(d)}}
function renderLoot(){const p=$('#lootPanel'),c=snap.openContainer;if(!c){p.classList.add('hidden');return}p.classList.remove('hidden');p.innerHTML=`<div style="display:flex;justify-content:space-between"><b>${c.kind==='corpse'?`시체 · ${c.ownerName}`:'보급 상자'}</b><button id="closeLoot">×</button></div><div class="meta" style="margin:5px 0">가까이 있어야 루팅창이 유지됩니다.</div><div id="lootRows"></div>`;$('#closeLoot').onclick=()=>session?.sendAction({type:ACTIONS.CLOSE_CONTAINER});const rows=p.querySelector('#lootRows');if(!c.items.length)rows.innerHTML='<div class="muted" style="padding:12px">비어 있음</div>';for(const it of c.items){const r=document.createElement('div');r.className='lootRow';r.innerHTML=`<div><b>${itemName(it)}</b><small>${itemSub(it)}</small></div>`;const b=document.createElement('button');b.textContent='획득';b.disabled=snap.player.inventory.length>=snap.player.bagCapacity;b.onclick=()=>session?.sendAction({type:ACTIONS.LOOT,itemId:it.id});r.appendChild(b);rows.appendChild(r)}}
...
function drawZones(cam){ctx.font='bold 34px monospace';ctx.fillStyle='rgba(210,218,218,.035)';for(const z of snap.zones)if(onScreen(z.x+z.w/2,z.y+z.h/2,cam,500))ctx.fillText(z.name,z.x+24,z.y+54)}
...
function drawObstacles(cam){for(const o of snap.obstacles){if(o.x+o.w<cam.x-40||o.x>cam.x+canvas.width+40||o.y+o.h<cam.y-40||o.y>cam.y+canvas.height+40)continue;ctx.fillStyle=obstacleColor(o.kind);ctx.fillRect(o.x,o.y,o.w,o.h);ctx.fillStyle='rgba(255,255,255,.055)';ctx.fillRect(o.x,o.y,o.w,3);ctx.strokeStyle='rgba(0,0,0,.42)';ctx.strokeRect(o.x,o.y,o.w,o.h)}}
function drawCrates(cam){for(const c of snap.crates){if(!onScreen(c.x,c.y,cam))continue;ctx.fillStyle=c.opened?'#403b31':'#79633b';ctx.fillRect(c.x-16,c.y-12,32,24);ctx.strokeStyle=c.empty?'#4d4d47':'#b39454';ctx.strokeRect(c.x-16,c.y-12,32,24);if(!c.opened){ctx.fillStyle='#c8a961';ctx.fillRect(c.x-10,c.y-3,20,3)}}}
function drawCorpses(cam){for(const c of snap.corpses){if(!onScreen(c.x,c.y,cam))continue;ctx.fillStyle='#302928';ctx.beginPath();ctx.ellipse(c.x,c.y,19,10,0,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#685a55';ctx.stroke()}}
